...
app.service('$service', ['$window', '$rootScope', '$http', '$localstorage', 'web', function ($window, $rootScope, $q, $localstorage, web) {
    ...
    this.getResultsFromStackFilter = function () {
        service.rest('resultsFromStackFilter', function (data) {
            console.log(data)
        }, "&aStackFilter[startTime]=" + service.roundTime(Math.floor(Date.now() / 1000)) + "&aStackFilter[endTime]=" + service.roundTime(Math.floor(Date.now() / 1000 + 1800)) + "&aStackFilter[latitude]=" + "43.467121" + "&aStackFilter[longitude]=" + "-80.546756" + "&includeStack=" + 'true');
    };
